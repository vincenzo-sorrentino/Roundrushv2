/**
 * components.js — Roundrush Marketing Website
 * Builds and injects the header and footer into every page.
 * Auth links point to the prototype Vite app dev server routes.
 */

// ── Config ──────────────────────────────────────────────────────
// Update these URLs to match your deployment setup.
const CONFIG = {
  authLoginUrl:  '/auth/login/default',
  authSignupUrl: '/auth/signup/team',
  baseUrl: '',  // Set to '' for root-relative paths
};

// ── Logo SVG ────────────────────────────────────────────────────
// `id` param ensures each instance uses a unique clip-path ID.
function logoSVG(id) {
  return `<svg width="213" height="32" viewBox="0 0 213 32" fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="Roundrush" role="img">
<g clip-path="url(#${id})">
...
<defs><clipPath id="${id}"><rect width="213" height="32" fill="white"/></clipPath></defs>
</svg>`;
}

// ── Nav links ────────────────────────────────────────────────────
const NAV_LINKS = [
  { href: '/website/pages/index.html',     label: 'Home',      match: 'index'     },
  { href: '/website/pages/features.html',  label: 'Features',  match: 'features'  },
  { href: '/website/pages/framework.html', label: 'Framework', match: 'framework' },
];
...
function buildHeader() {
  ...
  const navLinksHTML = NAV_LINKS.map(link => {
    const isActive = currentPage === link.match || (currentPage === '' && link.match === 'index');
    return `<a href="${link.href}" class="ws-nav-link${isActive ? ' is-active' : ''}">${link.label}</a>`;
  }).join('');

  const mobileNavLinksHTML = NAV_LINKS.map(link => {
    return `<a href="${link.href}" class="ws-mobile-nav-link">${link.label}</a>`;
  }).join('');
...
<header class="ws-header" role="banner">
  <div class="ws-container">
    <div class="ws-header-inner">
      ...
      <a href="/prototypes" class="ws-logo" aria-label="Roundrush home">
        ${logoSVG('wl-clip-header')}
      </a>
      ...
      <div class="ws-header-actions">
        <a href="${CONFIG.authLoginUrl}" class="ws-btn ws-btn--ghost">Login</a>
        <a href="${CONFIG.authSignupUrl}" class="ws-btn ws-btn--primary">Sign up</a>
      </div>
...
<div class="ws-mobile-nav" id="ws-mobile-nav" aria-hidden="true">
  <nav class="ws-mobile-nav-links" aria-label="Mobile navigation">
    ${mobileNavLinksHTML}
  </nav>
  <div class="ws-mobile-nav-actions">
    <a href="${CONFIG.authLoginUrl}" class="ws-btn ws-btn--outline ws-btn--full">Login</a>
    <a href="${CONFIG.authSignupUrl}" class="ws-btn ws-btn--primary ws-btn--full">Sign up</a>
  </div>
</div>`;
...
function buildFooter() {
  ...
  const footerHTML = `
<footer class="ws-footer" role="contentinfo">
  <div class="ws-container">
    <div class="ws-footer-inner">

      <div class="ws-footer-brand">
        <a href="/prototypes" class="ws-footer-logo" aria-label="Roundrush home">
          ${logoSVG('wl-clip-footer')}
        </a>
        <p class="ws-footer-tagline">
          A structured execution system for software teams and AI agents.
        </p>
      </div>

      <div class="ws-footer-col">
        <p class="ws-footer-col-title">Product</p>
        <ul class="ws-footer-links">
          <li><a href="../pages/features.html" class="ws-footer-link">Features</a></li>
          <li><a href="../pages/framework.html" class="ws-footer-link">Framework</a></li>
        </ul>
      </div>

      <div class="ws-footer-col">
        <p class="ws-footer-col-title">Company</p>
        <ul class="ws-footer-links">
          <li><a href="#" class="ws-footer-link">About</a></li>
        </ul>
      </div>

      <div class="ws-footer-col">
        <p class="ws-footer-col-title">Account</p>
        <ul class="ws-footer-links">
          <li><a href="${CONFIG.authLoginUrl}" class="ws-footer-link">Login</a></li>
          <li><a href="${CONFIG.authSignupUrl}" class="ws-footer-link">Sign up</a></li>
        </ul>
      </div>
    ...
    <div class="ws-footer-bottom">
      <p class="ws-footer-copyright">© ${year} Roundrush. All rights reserved.</p>
    </div>
  </div>
</footer>`;
